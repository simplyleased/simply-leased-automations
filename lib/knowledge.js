...
let CACHE = null;
let CACHE_MTIME = 0;
// index.md is rewritten on every regen — use it as a freshness sentinel so the
// portal picks up the nightly re-harvest without a restart.
function knowledgeMtime() {
  try { return fs.statSync(path.join(knowledgeRoot(), 'index.md')).mtimeMs; } catch { return 0; }
}
function loadChunks() {
  const mt = knowledgeMtime();
  if (CACHE && mt === CACHE_MTIME) return CACHE;
