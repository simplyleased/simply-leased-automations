...
// Last run for EVERY function keyed by slug (no top-N slice) — used to drive the
// per-card status on the dashboard from real audit data instead of hard-coded text.
export function lastRunBySlug(limit = 800) {
  const { events } = recentEvents(limit); // newest-first
  const out = {};
  for (const [slug, cfg] of Object.entries(FN)) {
    const ev = events.find((e) => (e.action || '').startsWith(cfg.prefix));
    if (ev) out[slug] = { ts: ev.ts, email: ev.email, state: /fail/i.test(ev.action) ? 'err' : 'ok' };
  }
  return out;
}
